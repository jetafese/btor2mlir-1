...
::mlir::Attribute BitVecAttr::parse(::mlir::AsmParser &parser, ::mlir::Type type) {
  ...
  if (failed(_result_type)) {
    parser.emitError(parser.getCurrentLocation(), "failed to parse BitVecAttribute parameter 'type' which is to be a `BitVecType`");
    return {};
  }
  // Parse literal '='
  ...
  if (failed(_result_value)) {
    parser.emitError(parser.getCurrentLocation(), "failed to parse BitVecAttribute parameter 'value' which is to be a `APInt`");
    return {};
  }
  // Parse literal '>'
  if (parser.parseGreater())
    return {};
  return BitVecAttr::get(parser.getContext(),
      _result_type.getValue(),
